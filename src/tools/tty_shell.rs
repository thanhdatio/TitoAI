            ...
            "read" => {
                ...
                let max_output =
                    args.get("max_output")
                        ...
                        .clamp(1, MAX_SESSION_OUTPUT_BYTES as u64) as usize;
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    #[tokio::test]
    async fn tty_shell_start_send_read_close_roundtrip() {
        let security = Arc::new(SecurityPolicy {
            autonomy: AutonomyLevel::Full,
        ...
        let start = tool.execute(json!({"action": "start"})).await.unwrap();
        assert!(start.success, "start error: {:?}", start.error);
        let start_json: serde_json::Value = serde_json::from_str(&start.output).unwrap();
        ...
        let sent = tool
            ...
            .await
            .unwrap();
        assert!(sent.success, "send error: {:?}", sent.error);
        ...
        for _ in 0..5 {
            let read = tool
                ...
                .await
                .unwrap();
            assert!(read.success, "read error: {:?}", read.error);

            let read_json: serde_json::Value = serde_json::from_str(&read.output).unwrap();
            let output = read_json["output"].as_str().unwrap_or("");
            combined_output.push_str(output);
            if combined_output.contains("tty-ok") {
                break;
            }
        }

        assert!(
            combined_output.contains("tty-ok"),
            "output was: {combined_output}"
        );
